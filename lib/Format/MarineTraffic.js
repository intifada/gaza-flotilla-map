...
OpenLayers.Format.MarineTraffic = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    *
    * Parameters:
    * segment - {<DOMElement>} a segment to parse
    * segmentType - {String} nodeName of positions that form the line
    *
    * Returns:
    * {<OpenLayers.Geometry.Collection>} A collection geometry
    */
    extractTrack: function(segment, segmentType) {
        var track = this.getElementsByTagNameNS(segment, segment.namespaceURI, segmentType);
        var point_features = this.extractPoints(track);
    ...
    *
    * Parameters:
    * track - {<DOMElement>} a track to parse
    *
    * Returns:
    * An Array of <OpenLayers.Geometry.Point>s
    */
    extractPoints: function(track) {
